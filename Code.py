...
win = pygame.display.set_mode((600,1080))  ##fentre de dimension 1920:980
...
bg = pygame.image.load('img/bg.jpg') ##fond de notre jeux 
...
walkLeft= [pygame.image.load('img/1.png'), pygame.image.load('img/2.png'), pygame.image.load('img/3.png'), pygame.image.load('img/4.png'), pygame.image.load('img/5.png'), pygame.image.load('img/6.png'), pygame.image.load('img/2.png'), pygame.image.load('img/3.png'), pygame.image.load('img/4.png'), pygame.image.load('img/5.png')]
walkRight = [pygame.image.load('img/L1.png'), pygame.image.load('img/L2.png'), pygame.image.load('img/L3.png'), pygame.image.load('img/L4.png'), pygame.image.load('img/L5.png'), pygame.image.load('img/L6.png'), pygame.image.load('img/L2.png'), pygame.image.load('img/L3.png'), pygame.image.load('img/L4.png'), pygame.image.load('img/L5.png')]
...
navir = pygame.image.load('img/A.png')  ## image du bateau 
...
def message (texte):
    ...
    GTexte = pygame.font.Font('font/Second.ttf',80)
    Ptexte = pygame.font.Font('font/Second.ttf ', 50)                        ## parametrage de la police font et taille du texte
    RetournerMenu = pygame.font.Font ('font/Second.ttf ',80)


    GtexteSurf,GTexteRect = creaTextObjet (texte, GTexte)               ## création d'un rectangle qui acceuil la zone de texte 
    GTexteRect.center = int(600/2) , (int(1080/2)-10)                   ## positionnement du texte 
    win.blit(GtexteSurf, GTexteRect)                                    ## affiche 
    PtexteSurf,PtexteRect = creaTextObjet("Roujé dans 5 secondes",Ptexte)   ##  <<
    PtexteRect.center = int(600 / 2) , (int(1080/ 2) + 30)                 ##  <<
    win.blit ( PtexteSurf, PtexteRect )                                     ##  <<
    RetourMenuSurf,RetourMenuRect = creaTextObjet('Home',RetournerMenu)     ##  <<
    RetournerMenu= 600 / 2 , ((1080/3) + 10)                               ##  <<
...
def fps_affiche ():
    ...
    Seconde_txt = pygame.font.SysFont('font/Second.ttf', 25)
    ...
    FPStexteRect.topleft = (500,1040)                                                              ## emplacement du texte
...
# A toi de changer les objets , de gérer les collisions et le score. Rien de compliqué ici je pense
     ...
     aleat=randint (1,1000)     #Affichage aléatoire des pommes en fonction de la position du navire
     if aleat < 20:       # entre 0 et 100. Plus on se rapproche de 100, plus il y aura des pommes
...
def corps_game():
    ...
    while run :
        ...
        if y_perso >=1020:
            y_perso= 1019                        ## on délimite la zone de jeux 
        if x_perso > 550:
            x_perso = 549
        ...
        if not(isJump):
            if keys[pygame.K_SPACE]:
                ...
                walkCount = 0                          
            if y_perso <= 300:
                y_perso =299                             ## on parametre le saut 
        elif y_perso >= 1080:
                y_perso = 1079
        else:
            if jumpCount >= -10:
